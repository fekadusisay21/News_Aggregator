...
export default function NewsCard({ NewsImg, NewsTitle, NewsDescription }) {
  ...
  return (
    ...
    <Card sx={{ maxWidth: 400 }}>
      ...
      />
      <Styled_card>
        <Styled_Topography gutterBottom variant="h5" component="div">
          {NewsTitle}
        </Styled_Topography>
        <Styled_Topography variant="body2">
          {NewsDescription}
        </Styled_Topography>
      </Styled_card>
      <IconContainer>
      {<Styled_share  onClick={isShared ? () => setIsShared(false): () => setIsShared(true)}/>}
   {<Styled_bookmark onClick = {isBookmarked ? () => setIsBookmarked(false): () => setIsBookmarked(true)}/>}
   {<Styled_favorite  onClick={isFavorite ? () => setIsFavorite(false): () => setIsFavorite(true)}/>}
      </IconContainer>
    </Card>
  );
  }